...
Read anything you are unsure of live (`GET departments`, `roles`, `custom_fields/1773`, `interview_kits?query=`, `GET jobs/{{TT_TEMPLATE_JOB_ID}}`). These are the known-stable ones.

## Template
- **Atomic Template job = {{TT_TEMPLATE_JOB_ID}}** (`template:true`, `human_status:template`). Read its stages, triggers, and `location_ids` live at build time.
