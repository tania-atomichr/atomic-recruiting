...
- **COVER IMAGE (fresh-POST jobs lose it) — copy it explicitly.** A public-POST job has NO hero image; the template's lives at `job_detail.image_with_setting.picked_image.image` (the "Job Hero Background.png", **image id 109692**). Set it in the configure PUT: `job_detail.image_with_setting_attributes:{id:<job's own iws id, from GET job_details>, picked_image_attributes:{image_id:109692, row_order:0}}`. Without this the posting looks blank/unbranded (Tania noticed immediately). Read the template's current picked_image id live in case the hero image changes.
- **TEMPLATE MEDIA + EMAILS + TEAM (fresh-POST jobs lose ALL of these — copy every one, verified live 2026-07-17):**
  - **Videos:** template job_detail carries `picked_videos` (job-hero videos). Copy with `job_detail.picked_videos_attributes:[{video_id:<video.id from template's picked_videos[].video.id>, lid:uuid()}]` → verified lands (2 videos).
  - **Auto-reply email** (`job_detail.reply_subject` + `reply_body`, "We have received your application!") and **auto-reject email** (`reject_subject` + `reject_body`): copy from template BUT the bodies carry the OLD role name HARDCODED — rewrite it to this role's name (regex both `&` and `&amp;` variants).
  - **Hiring team:** `job.team_member_ids` (template's array, e.g. 6 members) — copy as-is.
  - Job-level "Skills" on the Evaluation tab come from the ATTACHED INTERVIEW KIT (job-level scorecard_picks stay empty even on the template) — no separate copy needed, but the kit must be attached.
  - Verify after: videos count, reply/reject contain the new role name, team count, and the usual question-count + body-length checks.
