...
2. **Resolve the Tasks Tracker, then create the OB row from ITS template.**
   ...
   - **Create the OB row from that data source's page template named "Opp Brief - Role Name at Company Name"** (pass its `template_id`), with title property `Task name` = **"Opp Brief - {Role} at {Company}"**. Resolve that template id inside the resolved data source at runtime; it differs per role.
...
- **The Tasks Tracker is multi-source.** Two data sources are linked in that one block (a blank placeholder + the real "Tasks Tracker"). Reading the page's inline `<database>` tag gives you only one collection id and it may be the placeholder — fetch the database object and match on the source NAMED "Tasks Tracker" with the `Task name` schema.
